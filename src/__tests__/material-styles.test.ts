...
describe('material-styles 素材差异化', () => {
  ...
  it('snow 粒子层数 ≥28 且色相=210', () => {
    const ps = crackStyle('snow').emit(0, 0, DEFAULT_VEL);
    expect(ps.length).toBeGreaterThanOrEqual(28);
  ...
  it('tornado 粒子层数 ≥32 且色相=185', () => {
    const ps = crackStyle('tornado').emit(0, 0, DEFAULT_VEL);
    expect(ps.length).toBeGreaterThanOrEqual(32);
